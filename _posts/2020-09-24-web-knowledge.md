...
**A:** 我們可以把這串數字切成4等分，而每一等份長度都是8，而將每一等分的二進位字串用十進位數字來表達，就會變成一個0 ~ 255的數字，之後再把這4等分的數字用點連起來就變成了我們平常看到的IP的樣子。
...
**A:** 上面有提到IP是長度為32的一串數字，而被遮罩遮到的部份代表網路位址（郵局的地址），剩下沒被遮罩遮到的部份代表主機位址（也就是那間郵局之下某間住戶的地址）。沒被遮罩遮到的部份長度越長，就代表那間郵局之下有更多的住戶。
